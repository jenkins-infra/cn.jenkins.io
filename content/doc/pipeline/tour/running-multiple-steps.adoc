...
Jenkins Pipeline 提供了很多的步骤（steps），这些步骤可以相互组合嵌套，方便地解决像重复执行步骤直到成功（重试）和如果一个步骤执行花费的时间太长则退出（超时）等问题。
...
内嵌类型的步骤，例如 `timeout` and `retry` 可以包含其他的步骤，包括 `timeout` and `retry` 。
