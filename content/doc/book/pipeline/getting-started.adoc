...
正如 link:../[previously]提到的, Jenkins流水线是一套插件，它支持s实现和集成持续交付流水线到Jenkins。P流水线提供了一组可扩展的工具，用于通过流水线 DSL将简单到复杂的交付流水线建模为 "代码"。
...
. 从Jenkins 主页 (即Jenkins经典 UI 的仪表盘),
  点击左上的 *New Item*。
+
[.boxshadow]
...
* 通过经典的UI定义流水线可以很方便的测试流水线代码片段, 或处理简单的流水线或 不需要从仓库中检出/clone源代码的流水线。 正如上面提到的, 不像通过Blue Ocean
  ...
  (link:#defining-a-pipeline-in-scm[below])定义的`Jenkinsfile`&zwj;s , `Jenkinsfile`&zwj;s 进入流水线项目的*Script* 文本区域，
