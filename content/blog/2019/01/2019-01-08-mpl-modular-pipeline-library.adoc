...
尽管通过自动化部署加快了开发速度，但由于在 DevOps 方面缺少协作，我们其中一个客户的上市时间因此被拖慢。虽然他们投入资源到 DevOps 中，但是每条生产流水线（production pipeline）都是独立的，迫使团队为每个项目重新制作轮子。更糟糕的是，由于没有跨团队协作，平台中的任何错误又会出现在每条新的流水线中。我们的很多客户都有类似的问题，所以我们决定开发一个既能帮助现有客户又能适应未来的通用工具。虽然使用通用框架且的标准化 CI/CD 平台是最明显的选择，但这会导致缺少灵活性的单体结构（monolithic structure），最后会变得举步维艰。由于每个团队都需要工作在自己的流水线上，我们开发了一个解决方案，方便 DevOps 流水线的每个可重用部分供以后使用：Jenkins 驱动的模块化流水线库。
...
首先，让我们介绍构建 MPL 时使用到的技术。

=== 使用共享库和 Jenkins 流水线构建 MPL

我们的 Jenkins 自动化平台最近收到了一些 Jenkins 流水线的更新。这些更新允许我们创建一个 Jenkinsfile 文件来描述整条流水线，以及用于执行一系列一目然的脚本。这提高了最终用户对 CI/CD 自动化流程的可视化程度，并提高了 DevOps 团队对流水线的可支持性。

然而，流水线存在一个很大的问题：很难用唯一的流水线支持多个 Jenkinsfile 文件（因此存在多少个项目就存在多少个 Jenkinsfile
文件）。我们需要一个地方存放公共逻辑，Jenkins 的共享库能帮我们实现。共享库用于存放流水线公共的部分，它定义在 Jenkinsfile 文件中，允许在 Jenkinsfile 中使用接口简化自动化脚本。
...
MPL中还有许多其他功能，但它本质上是一个解决一般 DevOps 协作问题的平台。为了简化开发和手动测试，MPL 提供了模块覆盖和继承模型，允许用户在不影响其他任何情况下测试项目中的特定修复。在 Jenkins 中，一个模块就是一个文件，其中包含脚本步骤和逻辑，以实现简单的目标（构建工件，运行测试，创建图像等）。这些模块在流水线的阶段中可以被组合使用，而且任何了解 Jenkins 流水线语法的人都可以轻松读懂。
...
假设你有一个常见的Java Maven项目。你在项目中创建 Jenkinsfile，并希望使用 DevOps 团队准备的默认流水线。MPL 本身就提供一个简单的流水线：核心 MPLPipeline 。这是一个非常简单的流水线，但对于想要尝试 MPL 的人来说，这是一个很好的开端。我们来看一下这个简单的 Jenkinsfile 文件：
...
在部署阶段之后，流水线会执行测试阶段，但是在测试阶段已经没有什么好介绍的了。然而，测试中有一个非常重要的事情，那就是 MPL 本身的测试。
